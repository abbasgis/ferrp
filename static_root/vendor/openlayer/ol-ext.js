...
ol.control.Bar.prototype.onActivateControl_ = function (e) {
    if (this.get('toggleOne')) {
        ...
        else {	// No one active > test auto activate
...
ol.control.Overlay = function (options) {
    ...
    var self = this;
    if (options.hideOnClick) element.click(function () {
        self.hide();
    });
 ...
 * @private
 */
ol.control.Permalink.prototype.layerChange_ = function (e) {	// Get layers
    var l = "";
...
ol.control.Profil = function (opt_options) {
    ...
    this.bar_ = $("<div>").addClass("ol-profilbar")
        .css({
            top: (this.margin_.top / ratio) + "px",
            height: (this.canvas_.height - this.margin_.top - this.margin_.bottom) / ratio + "px"
        })
        .appendTo(div);
...
(function () {
    ...
    function addFilter_(filter) {
        ...
        if (filter.precompose) filter._listener.push({
            listener: this.on('precompose', precompose_.bind(filter)),
            target: this
        });
        if (filter.postcompose) filter._listener.push({
            listener: this.on('postcompose', postcompose_.bind(filter)),
            target: this
        });
...
ol.filter.Colorize.prototype.postcompose = function (e) {	// Set back color hue
    var ctx = e.context;
    var canvas = ctx.canvas;
    ctx.save();
 ...
 * @private
 */
ol.interaction.DrawHole.prototype._finishDrawing = function (e) {	// The feature is the hole
    e.hole = e.feature;
...
ol.interaction.DrawRegular.prototype.getGeom_ = function () {
    ...
    if (this.coord_) {
        ...
        if (!this.sides_ && this.square_ && !this.centered_) {
            ...
            var d = [coord[0] - center[0], coord[1] - center[1]];
            if (this.square_ && !hasrotation) {	//var d = [coord[0] - center[0], coord[1] - center[1]];
            ...
            if (r > 0) {
                ...
                else {	// Optimize points on the circle
    ...
    switch (evt.type) {
        ...
        case "pointerup":
            ...
            if (this.started_ && this.coord_) {
                ...
                if (dx * dx + dy * dy <= this.squaredClickTolerance_) {	// The pointer has moved
        ...
        default:
            ...
            if (this.started_ && evt.type === 'dblclick') {	//evt.stopPropagation();
                return false;
            }
            break;
    }
...
ol.interaction.DropFile = function (options) {
    ...
    $(zone).on('drop', function (e) {
        return self.ondrop(e.originalEvent);
    });
};
...
ol.interaction.GeolocationDraw.prototype.draw_ = function (active) {
    ...
    switch (this.get('followTrack')) {	// Follow center + zoom
        case true:
            // modify zoom
        ...
        case 'auto':
            if (this.lastPosition_) {
                ...
                if (center[0] != this.lastPosition_[0] || center[1] != this.lastPosition_[1]) {	//this.dispatchEvent({ type:'follow', following: false });
 ...
 * @return {boolean} `true` to start the drag sequence.
 */
ol.interaction.Split.prototype.handleDownEvent = function (evt) {	// Something to split ?
 ...
 * @API stable
 */
ol.source.DBPedia.prototype.readFeature = function (feature, attributes, lastfeature) {	// Copy RDF attributes values
    for (var i in attributes) feature.set(i, attributes[i].value);
    // Prevent same feature with different type duplication
    if (lastfeature && lastfeature.get("subject") == attributes.subject.value) {	// Kepp dbpedia.org type ?
 ...
 * @api stable
 */
ol.source.GeoImage.prototype.setCrop = function (crop) {	// Image not loaded => get it latter
    ...
    if (a.start) {
        ...
        for (var i = 0, c; c = a.clusters[i]; i++) {
            ...
            for (var k = 0, s; s = st[k]; k++) {
                ...
                // OL3 > v3.14
                if (vectorContext.setStyle) {	// If one feature: draw the feature
...
/**
 */
ol.render3D.prototype.drawFeature3D_ = function (ctx, build) {	// Construct
...
ol.Overlay.Popup.prototype.show = function (coordinate, html) {
    ...
    var map = this.getMap();
    if (!map) return;
    if (html && html !== this.prevHTML) {	// Prevent flickering effect
        this.prevHTML = html;
    ...
    if (coordinate) {	// Auto positionning
...
ol.style.Chart = function (opt_options) {
    ...
    } : this.animation_ = {animate: false, step: 1};
...
ol.style.Chart.colors =
    {
        ...
        "ferrp": ["#c00000", "#1b00e3", "#06b949", "#ff376b", "#d7a200", "#ff6600", "#019084", "#770190",
                "#000000", "#9999cc", "#999933", "#996633", "#ff9933", "#ffcc33", "#FF0000"]
    }
/**
 * Clones the style.
