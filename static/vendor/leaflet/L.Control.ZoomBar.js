...
L.Control.ZoomBar = L.Control.extend(
{
    ...
    onAdd: function(map)
    {
        ...
        // if( this._isSmallScreen()===false ){
        //     this._drawPolylineButton = this._createButton('', 'Draw Polyline',
        //         'leaflet-control-drawline', container, this._drawLine, this);
        // }
    ...
    _drawLine:function (e) {
        ...
        this._map.on('mousedown', function (e) {
            _lineBounds.push(e.latlng);// = [e.latlng];

            this._zoomRect = new L.polyline(_lineBounds,
                    this._xsStyle).addTo(this._map);

            });
        this._map.on('dblclick', this._stopElastic, this);
        //
        // this._map.dragging.disable();
    },
    _startLineElastic:function (e) {
        this._lineBounds = [e.latlng];

        this._zoomRect = new L.polyline(this._lineBounds,
                this._xsStyle).addTo(this._map);
